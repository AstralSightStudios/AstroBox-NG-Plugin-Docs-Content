...
title: 社区与社交功能报错
icon: Users
description: 资源评论、评分、举报、收件箱等社交功能可能出现的报错、解决方案及说明
---
...
```text
你的社交功能已被限制，解除时间：{expiresAt}。{error.message}
```

**触发场景**：后端返回 `social-banned` 错误码，账号社交功能被临时限制。
...
```text
账号已被封禁：{error.message}
```

**触发场景**：后端返回 `user-banned` 错误码，账号被封禁，随后会自动登出。
...
<a id="comment-permission-hint" />

**报错原文**

```text
resource.comment.permissionHint
```

**触发场景**：无评论权限时尝试打开回复对话框。
...
<a id="board-pro-required" />

**报错原文**

```text
profile.edit.board.proRequired
```
...
<a id="queue-device-unknown" />

**报错原文**

```text
profile.collections.queueDeviceUnknown
```

**触发场景**：收藏集批量入队时无法解析设备 ID。
...
<a id="officialv2-map-not-ready" />

**报错原文**

```text
OfficialV2 device map is not ready
```

**触发场景**：OfficialV2 设备映射未加载完成即发起操作。
