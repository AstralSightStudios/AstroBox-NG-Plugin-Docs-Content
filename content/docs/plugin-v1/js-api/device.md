...
| **MiWearBleCharaUuid** | `recv: string`<br />`sent: string` | BLE 特征值 UUID（接收/发送）（仅 macOS / iOS 可用） |
| **SecurityKeys** | `enc_key: number[]`<br />`dec_key: number[]`<br />`enc_nonce: number[]`<br />`dec_nonce: number[]` | 一次性 Session 密钥（对插件来说没什么卵用） |
